...
type TrendingCardProps = {
  ...
  const TrendingCard = ({ className, post }: TrendingCardProps) => {
    return (
      <Link
      className={`${className} sm:mt-0 sm:h-auto relative mt-7 block w-full h-96 hover:opacity-90 hover:text-black`}
    ...
    >
      ...
      <div className="absolute z-2 bottom-0 left-0 p-3">
        <h4 className="inline-block bg-red-500 text-white px-5 py-1 font-semibold  text-wh-900">
          {post?.category}
        </h4>
